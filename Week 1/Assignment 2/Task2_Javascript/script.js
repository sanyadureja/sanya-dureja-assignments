...
let transactions = []; // task 1 completed
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    const reason = document.getElementById("reason").value; // task 2 completed
...
function addTransaction(payee, amount, reason) {
  ...
  transactions.push(newTransaction); // task 4 completed
}
...
function updateTransaction(id, payee, amount, reason) {
  ...
  const transaction = transactions.find((t) => t.id === id); // task 5 completed
...
function deleteTransaction(id) {
  ...
  if (index !== -1) {
    ...
    transactions.splice(index, 1); // task 6 completed
